...
class mzitu():
    ...
    def mkdir(self, path): ##这个函数创建文件夹
        path = path.strip()
        isExists = os.path.exists(os.path.join("E:\wsk\Python\pachong\爬虫\Study\百度图片", path))
        if not isExists:
            print(u'建了一个名字叫做', path, u'的文件夹！')
            os.makedirs(os.path.join("E:\wsk\Python\pachong\爬虫\Study\百度图片", path))
            os.chdir(os.path.join("E:\wsk\Python\pachong\爬虫\Study\百度图片", path)) ##切换到目录
            return True
        else:
